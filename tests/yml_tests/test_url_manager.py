...
class test_URL_Manager:
    def __init__(self, client: TestClient, **args):
        ...
        full_url = "/".join(
            s.strip("/") for s in url_parts
        )  # If both/neither have '/' between them, this still joins them correctly
    ...
    def runQuery(self):
        def countToDict(html):
            try:
                ...
                assert False, self.error_msg.format(
                    "API returned html that was not a count. (Error page?) \nHTML Page (First 500 chars): \n{0}\n".format(
                        file_content[:500]
                    )
                )
            return {"count": count}
        ...
        def csvToDict(file_content):
            ...
            assert len(rotated_content) > 1, self.error_msg.format(
                "API did not return a CSV.\nReturned content: (First 500 chars): \n{0}\n".format(
                    file_content[:500]
                )
            )
            file_content = {}
        ...
        def downloadToDict(bulk_download_file):
            ...
            if files == None:
                assert False, self.error_msg.format(
                    "Problem reading download script! \nReturned content: (First 500 chars): \n{0}\n".format(
                        bulk_download_file[:500]
                    )
                )
        ...
        def asfSearchToDict(asf_search_file):
            ...
            geojson_data = outputs.get(
                "results", outputs.get("stack", asf_search.ASFSearchResults([]))
            ).geojson()
            ...
            assert script_geojson == str(geojson_api), (
                "asf-search python file output differed from equivalent api geojson output"
            )
        ...
        try:
            ...
            assert False, self.error_msg.format(
                "Server returned no info. Normally means it's overloaded."
            )
        ...
        try:
            ...
            assert False, self.error_msg.format(
                "Header is not formatted as expected. Header: {0}.\nFile Content (First 500 char): \n{1}\n".format(
                    content_header, file_content[:500]
                )
            )
        ...
        ## JSON / JSONLITE / ERROR
            ...
            try:
                ...
                self.error_msg.format(
                    "Test returned json header, but content failed to load.\nContent (First 500 char):\n{0}\n".format(
                        file_content
                    )
                )
            ## ERROR
            ...
            ## JSON
                ...
                assert isinstance(file_content, type([])), self.error_msg.format(
                    "Response did not contain a list of results.\nContent (First 500 char):\n{0}\n".format(
                        file_content
                    )
                )
    ...
    def runAssertTests(self, test_info, status_code, content_type, file_content):
        if "expected code" in test_info:
            assert test_info["expected code"] == status_code, self.error_msg.format(
                "Status codes is different than expected."
            )
        if "count" in file_content and "maxResults" in test_info:
            assert test_info["maxResults"] >= file_content["count"], self.error_msg.format(
                "API returned too many results."
            )
        if "expected file" in test_info:
            assert test_info["expected file"] == content_type, self.error_msg.format(
                "Different file type returned than expected."
            )
                ...
                if key in test_info and key in file_dict:
                    ...
                    for found_param in file_dict[key]:
                        ...
                        for poss_list in test_info[key]:
                            ...
                            if isinstance(poss_list, type([])):
                                ...
                                if found_param.startswith("[") and found_param.endswith("]"):
                                    ...
                                    expect_type(found_param) >= poss_list[0]
                                    and expect_type(found_param) <= poss_list[1]
                                ):
                                    found_in_list = True
                                    break
                            # This part gets hit for single numbers, and strings. (ie "Platform"):
                                ...
                                if (
                                    isinstance(found_param, str)
                                    and found_param.startswith("[")
                                    and found_param.endswith("]")
                                ):
                                    # expect_type = type(poss_list)
                                    found_param = ast.literal_eval(found_param)
                                    for param in found_param:
                                        if param == poss_list:
                                            ...
                                            found_in_list = True
                                            break
                                elif isinstance(found_param, list) or isinstance(
                                    found_param, tuple
                                ):
                                    for param in found_param:
                                        if param == poss_list:
                                    ...
                                    found_in_list = True
                                    break
                                elif isinstance(poss_list, tuple):
                                    if isinstance(found_param, str):
                                        r = range(int(poss_list[0]), int(poss_list[1]) + 1)
                                        try:
                                            if int(found_param) in r:
                                                found_in_list = True
                                                break
                                        except:
                                            if (
                                                float(poss_list[0]) <= float(found_param)
                                                and float(found_param) <= float(poss_list[1]) + 1
                                            ):
                                                found_in_list = True
                                                break
                        ...
                        if found_in_list == True:
                            break
                    assert found_in_list, self.error_msg.format(
                        key
                        + " declared, but not found in file. File contents for key:\n{0}\n".format(
                            file_dict[key]
                        )
                    )
            ...
            def checkMinMax(key, test_info, file_dict):
                if "min" + key in test_info and key in file_dict:
                    for value in file_dict[key]:
                        ...
                        assert number_type(value) >= test_info["min" + key], self.error_msg.format(
                            "Value found smaller than min key."
                        )
                if "max" + key in test_info and key in file_dict:
                    for value in file_dict[key]:
                        number_type = type(test_info["max" + key])
                        assert number_type(value) <= test_info["max" + key], self.error_msg.format(
                            "Value found greater than max key."
                        )
            ...
            def convertTimezoneUTC(time, tz_orig=None):
                ...
                tz_orig = get_localzone() if tz_orig == None else ZoneInfo(tz_orig)
                ...
                if isinstance(time, type("")):
                    ...
                    time = (
                        time[:-1] if time.endswith("Z") else time
                    )  # take off the 'Z' if it's on the end
                    time = (
                        time[:-3] if time.endswith("UTC") else time
                    )  # take off the 'UTC' if it's on the end
                    # Convert to a datetime object:
                    time = datetime.strptime(time, "%Y-%m-%dT%H:%M:%S")

                # Set the timezone to where the timestamp came from:
                time = time.astimezone(tz_orig)
            ...
            def checkDate(later_date=None, earlier_date=None):
                ...
                if isinstance(later_date, type([])):
                    ...
                    for theDate in later_date:
                        ...
                        assert theDate >= earlier_date, self.error_msg.format(
                            "File has too small of a date. File: {0}, earlier than test date: {1}.".format(
                                theDate, earlier_date
                            )
                        )
                    ...
                    for theDate in earlier_date:
                        theDate = convertTimezoneUTC(theDate, tz_orig="UTC")
                        assert later_date >= theDate, self.error_msg.format(
                            "File has too large of a date. File: {0}, later than test date: {1}.".format(
                                theDate, later_date
                            )
                        )
                    ...
                    earlier_date = convertTimezoneUTC(earlier_date)
                    later_date = convertTimezoneUTC(later_date)
                    assert later_date >= earlier_date, self.error_msg.format(
                        "Date: {0} is earlier than date {1}.".format(later_date, earlier_date)
                    )
            ...
            def checkSeason(file_start_dates, file_end_dates, season_list):
                ...
                if len(file_start_dates) == len(file_end_dates):
                    ...
                    assert False, self.error_msg.format(
                        "Error running test! Not same number of start and end dates."
                    )
                ...
                for date in file_dates:
                    ...
                    for season in season_list:
                        for the_range in days_ranges:
                            ...
                            if (season[0] <= the_range[0] <= season[1]) or (
                                season[0] <= the_range[1] <= season[1]
                            ):
                                season_range_hit = True
                                break

                    assert season_range_hit, self.error_msg.format(
                        "Seasons not found in file. file ranges: {0}. yml range: {1}.".format(
                            days_ranges, season_list
                        )
                    )
        ...
        try:
            ...
            for key, val in test_info.items():
                ...
                if key.lower() == "absoluteorbit":
                    ...
                    mutatable_dict["absoluteOrbit"] = string_to_obj_map[
                        asf_search.validators.parse_int_or_range_list
                    ](val)
                elif key.lower() == "platform":
                    del mutatable_dict[key]
                    mutatable_dict["Platform"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() in ["frame", "asfframe"]:
                    del mutatable_dict[key]
                    mutatable_dict["asfframe"] = string_to_obj_map[
                        asf_search.validators.parse_int_or_range_list
                    ](val)
                elif key.lower() == "granule_list":
                    del mutatable_dict[key]
                    mutatable_dict["granule_list"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "groupid":
                    del mutatable_dict[key]
                    mutatable_dict["groupid"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "flightdirection":
                    del mutatable_dict[key]
                    mutatable_dict["flightdirection"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "offnadirangle":
                    del mutatable_dict[key]
                    mutatable_dict["offnadirangle"] = string_to_obj_map[
                        asf_search.validators.parse_float_or_range_list
                    ](val)
                elif key.lower() == "polarization":
                    del mutatable_dict[key]
                    mutatable_dict["polarization"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "relativeorbit":
                    del mutatable_dict[key]
                    mutatable_dict["relativeorbit"] = string_to_obj_map[
                        asf_search.validators.parse_int_or_range_list
                    ](val)
                elif key.lower() == "collectionname":
                    del mutatable_dict[key]
                    mutatable_dict["collectionname"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() in ["beammode", "beamswath"]:
                    del mutatable_dict[key]
                    mutatable_dict["beammode"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "processinglevel":
                    del mutatable_dict[key]
                    mutatable_dict["processinglevel"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "flightline":
                    del mutatable_dict[key]
                    mutatable_dict["flightline"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "lookdirection":
                    del mutatable_dict[key]
                    mutatable_dict["lookdirection"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                elif key.lower() == "instrument":
                    del mutatable_dict[key]
                    mutatable_dict["instrument"] = string_to_obj_map[
                        asf_search.validators.parse_string_list
                    ](val)
                # elif key.lower() == "processingdate":
                #     del mutatable_dict[key]
                #     mutatable_dict["processingdate"] = asf_search.validators.parse_date(val.replace("+", " "))
                elif key.lower() == "start":
                    del mutatable_dict[key]
                    mutatable_dict["start"] = asf_search.validators.parse_date(
                        val.replace("+", " ")
                    )
                elif key.lower() == "end":
                    del mutatable_dict[key]
                    mutatable_dict["end"] = asf_search.validators.parse_date(val.replace("+", " "))
                elif key.lower() == "season":
                    del mutatable_dict[key]
                    mutatable_dict["season"] = asf_search.validators.parse_int_list(
                        string_to_obj_map[asf_search.validators.parse_int_list](val)
                    )
                # MIN/MAX variants
                ...
                # min/max Doppler:
                    ...
                    del mutatable_dict[key]
                    mutatable_dict[key.lower()[0:3] + "doppler"] = (
                        asf_search.validators.parse_float(val)
                    )
                # min/max InsarStackSize:
                elif key.lower()[3:] == "insarstacksize":
                    del mutatable_dict[key]
                    mutatable_dict[key.lower()[0:3] + "insarstacksize"] = (
                        asf_search.validators.parse_int(val)
                    )
                # min/max FaradayRotation:
                elif key.lower()[3:] == "faradayrotation":
                    del mutatable_dict[key]
                    mutatable_dict[key.lower()[0:3] + "faradayrotation"] = (
                        asf_search.validators.parse_float(val)
                    )
        ...
        if "Platform" in json_dict:
            ...
            for i, platform in enumerate(itter_copy["Platform"]):
                ...
                # Sentinel-1:
                    ...
                    json_dict["Platform"].append("Sentinel-1D")
                ...
                # Sentinel-1B
                    ...
                    json_dict["Platform"].append("Sentinel-1 Interferogram (BETA)")
                elif platform in ["SENTINEL-1D", "SD"]:
                    json_dict["Platform"][i] = "Sentinel-1D"
                    json_dict["Platform"].append("Sentinel-1 Interferogram (BETA)")
                # Sir-C
