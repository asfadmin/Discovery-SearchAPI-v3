                                ...
                                if isinstance(found_param, str) and found_param.startswith('[') and found_param.endswith(']'):
                                    ...
                                    for param in found_param:
                                        ...
                                        # if expect_type(param) >= poss_list[0] and expect_type(param) <= poss_list[1]:
                                            found_in_list = True
                                            break
                                elif isinstance(found_param, list) or isinstance(found_param, tuple):
                                    for param in found_param:
                                        if param == poss_list:
                                            found_in_list = True
                                            break
                                    ...
                                    if isinstance(found_param, str):
                                        ...
                                        try:
                                            ...
                                            if float(poss_list[0]) <= float(found_param) and float(found_param) <= float(poss_list[1]) + 1:
                                                found_in_list = True
                                                break
        ...
        try:
            ...
            for key, val in test_info.items():
                ...
                if key.lower() == "absoluteorbit":
                    ...
                    mutatable_dict["absoluteOrbit"] = string_to_obj_map[asf_search.validators.parse_int_or_range_list](val)
                elif key.lower() == "platform":
                    del mutatable_dict[key]
                    mutatable_dict["Platform"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() in ["frame", "asfframe"]:
                    del mutatable_dict[key]
                    mutatable_dict["asfframe"] = string_to_obj_map[asf_search.validators.parse_int_or_range_list](val)
                elif key.lower() == "granule_list":
                    del mutatable_dict[key]
                    mutatable_dict["granule_list"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "groupid":
                    del mutatable_dict[key]
                    mutatable_dict["groupid"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "flightdirection":
                    del mutatable_dict[key]
                    mutatable_dict["flightdirection"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                    ...
                    del mutatable_dict[key]
                    mutatable_dict["polarization"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "relativeorbit":
                    del mutatable_dict[key]
                    mutatable_dict["relativeorbit"] = string_to_obj_map[asf_search.validators.parse_int_or_range_list](val)
                elif key.lower() == "collectionname":
                    del mutatable_dict[key]
                    mutatable_dict["collectionname"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() in ["beammode", "beamswath"]:
                    del mutatable_dict[key]
                    mutatable_dict["beammode"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "processinglevel":
                    del mutatable_dict[key]
                    mutatable_dict["processinglevel"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "flightline":
                    del mutatable_dict[key]
                    mutatable_dict["flightline"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "lookdirection":
                    del mutatable_dict[key]
                    mutatable_dict["lookdirection"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                elif key.lower() == "instrument":
                    del mutatable_dict[key]
                    mutatable_dict["instrument"] = string_to_obj_map[asf_search.validators.parse_string_list](val)
                ...
                #     mutatable_dict["processingdate"] = asf_search.validators.parse_date(val.replace("+", " "))
                    ...
                    del mutatable_dict[key]
                    mutatable_dict["start"] = asf_search.validators.parse_date(val.replace("+", " "))
                elif key.lower() == "end":
                    del mutatable_dict[key]
                    mutatable_dict["end"] = asf_search.validators.parse_date(val.replace("+", " "))
                elif key.lower() == "season":
                    del mutatable_dict[key]
                    mutatable_dict["season"] = asf_search.validators.parse_int_list(string_to_obj_map[asf_search.validators.parse_int_list](val))
                # MIN/MAX variants
                # min/max BaselinePerp
                elif key.lower()[3:] == "baselineperp":
                    del mutatable_dict[key]
                    # Save the min/max key, all lower
                    mutatable_dict[key.lower()[0:3]+"baselineperp"] = string_to_obj_map[asf_search.validators.parse_float_or_range_list](val)
                # min/max Doppler:
                elif key.lower()[3:] == "doppler":
                    del mutatable_dict[key]
                    mutatable_dict[key.lower()[0:3]+"doppler"] = asf_search.validators.parse_float(val)
                # min/max InsarStackSize:
                elif key.lower()[3:] == "insarstacksize":
                    del mutatable_dict[key]
                    mutatable_dict[key.lower()[0:3]+"insarstacksize"] = asf_search.validators.parse_int(val)
                #min/max FaradayRotation:
                elif key.lower()[3:] == "faradayrotation":
                    del mutatable_dict[key]
                    mutatable_dict[key.lower()[0:3]+"faradayrotation"] = asf_search.validators.parse_float(val)
